...
public class BuyCar {
    ...
    public static final double INCREASE_OF_LOSS = 0.5;
    ...
    public static int[] nbMonths(int startPriceOld, int startPriceNew, int savingPerMonth, double percentLossByMonth) {
        ...
        if (oldPrice >= newPrice) {
            return new int[] {months, (int) Math.abs(oldPrice - newPrice)};
        }

        while ((oldPrice + savings) < newPrice) {
            months++;
            if (months %2 == 0) {
                percentLossByMonth += INCREASE_OF_LOSS;
            }
        ...
        return new int[] {months, (int) Math.round(savings + oldPrice - newPrice)};
    }
}
